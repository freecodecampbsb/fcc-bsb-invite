      ...
      request.post({
          ...
          if (body.ok) {
            res.render('result', {
              ...
              message: 'Yeahhh, Camper! Enviamos o convite para o e-mail &ldquo;'+ req.body.email +'&rdquo;.'
            });
          } else {
            ...
            if (error === 'already_invited' || error === 'already_in_team') {
              res.render('result', {
                community: config.community,
                message: 'Camper, você já foi convidado.<br>' +
                        'Visite <a href="https://'+ config.slackUrl +'">'+ config.community +'</a>'
              });
              return;
            } else if (error === 'invalid_email') {
              error = 'Ops! O e-mail enviado é inválido.';
            } else if (error === 'invalid_auth') {
              error = 'Algo deu errado! Por favor, contate o administrador ou tente novamente mais tarde.';
            }

            res.render('result', {
              community: config.community,
              message: 'Falhou! ' + error,
              isFailed: true
            });
          }
        });
